/**
 * Code-Synapse CLI Main Module
 * User-facing command line interface for configuration and server management
 *
 * Note: This module is loaded via bootstrap.ts which sets up max listeners
...
program
  ...
  .option("-m, --model <preset>", "LLM model preset (fastest, minimal, balanced, quality, maximum)")
  .option("-p, --provider <name>", "Override model provider (local, openai, anthropic, google)")
  .option("--model-id <id>", "Override model ID for this run")
...
function handleError(error: unknown): void {
  if (error instanceof Error) {
    ...
    logger.error(
      { unknownError: errorString, errorType: typeof error },
      "Unknown CLI error occurred: %s",
      errorString
    );
...
function registerProcessListeners(): void {
  ...
  // Max listeners already set at module level (top of file)
